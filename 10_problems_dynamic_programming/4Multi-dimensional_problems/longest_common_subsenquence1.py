...
class Solution:
    def longestCommonSubsequence(self, text1: str, text2: str) -> int:
        @cache
        def dp(i, j):
            # Base case: If either index is -1, we've reached the start of one string
            if i == -1 or j == -1:
                return 0

            # If characters match, move diagonally in the grid (i-1, j-1)
            if text1[i] == text2[j]:
                return 1 + dp(i - 1, j - 1)

            # If characters do not match, explore both possibilities:
            # 1. Move up in the grid (i-1, j)
            # 2. Move left in the grid (i, j-1)
            return max(dp(i - 1, j), dp(i, j - 1))

        # Start the dynamic programming recursion from the last characters of both strings
