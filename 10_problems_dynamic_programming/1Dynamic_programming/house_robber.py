...
class Solution:
    def __init__(self):
        # Memoization dictionary to store the maximum profit from index 'i' onwards
        self.memo = {}

    def rob(self, nums: List[int]) -> int:
        # Clear memoization dictionary for each test case
        self.memo = {}
        # Start from the first house (index 0)
        return self.robFrom(0, nums)

    def robFrom(self, i, nums):
        # Base case: If there are no more houses to examine, return 0.
        if i >= len(nums):
            return 0

        # If we've already computed the maximum profit for this house, return it from memo.
        if i in self.memo:
            return self.memo[i]

        # Recursive relation: Either skip this house or rob it and move to the next possible house.
        ans = max(
            # Option 1: Skip the current house, move to the next house (i + 1)
            self.robFrom(i + 1, nums),
            # Option 2: Rob the current house, skip the next house (i + 2)
            self.robFrom(i + 2, nums) + nums[i]
        )

        # Store the computed result in the memo dictionary for future reference.
        self.memo[i] = ans
        return ans
